...
$(document).ready(function() {
    ...
    $('.container-fluid').on('click','button.objInfo' , function(){
        ...
        $(id).find('.colorCodes').toggle("fast");
        $(id).find('.show').toggle();
...
//  add colors in card :
//  STILL TO IMPLEMENT:
//  copy color scheme to clipboard or add to a tweet
//
    var dataClipboardColor = "";
    var colorP = '<p class="colorCodes" data-clipboard-text="';
    var span = '';
    obj.colors.forEach(function(color){
        dataClipboardColor += color.originalHex + " ";
        span += '<span style="background-color:'+ color.originalHex +'; width= 40px; display:block">&nbsp;</span>';
    });

    colorP += dataClipboardColor + '>';
    colorP += span;
    colorP += '</p>';
    var html = "";
